...
void ANNkd_split::ann_search(ANNdist box_dist)
{
	...
	if (cut_diff < 0) {					// left of cutting plane
		...
		if (box_dist * ANNkdMaxErr < ANNkdPointMK->max_key() || isNearlyEqual(box_dist*ANNkdMaxErr, ANNkdPointMK->max_key()))
...
void ANNkd_leaf::ann_search(ANNdist box_dist)
{
	...
	for (int i = 0; i < n_pts; i++) {	// check points in bucket
		...
		for(d = 0; d < ANNkdDim; d++) {
			...
			if( (dist = ANN_SUM(dist, ANN_POW(t))) > min_dist && !isNearlyEqual(box_dist*ANNkdMaxErr, ANNkdPointMK->max_key())) {
				break;
			}
		}
