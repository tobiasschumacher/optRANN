	...
	double				eps)			// error bound (ignored)
{
	...
	ANNprPointMK = rnd_tb ? new ANNmin_k_tb(k) : new ANNmin_k(k);		// create set for closest k points according to tie break settings
	...
	while (ANNprBoxPQ->non_empty() &&
		...
		ANN_FLOP(2)						// increment floating ops
		if (box_dist*ANNprMaxErr > ANNprPointMK->max_key() && !isNearlyEqual(box_dist*ANNprMaxErr, ANNprPointMK->max_key())) // also allow ties for random tie breaks
			break;
...
void ANNkd_leaf::ann_pri_search(ANNdist box_dist)
{
	...
	for (int i = 0; i < n_pts; i++) {	// check points in bucket
		...
		for(d = 0; d < ANNprDim; d++) {
			...
			if( (dist = ANN_SUM(dist, ANN_POW(t))) > min_dist && !isNearlyEqual(dist, min_dist)) {
				break;
			}
		}
