	...
	double				eps)			// the error bound
{
	...
	ANNkdFRPointMK = rnd_tb ? new ANNmin_k_tb(k) : new ANNmin_k(k);		// create set for closest k points according to tie break settings
...
void ANNkd_split::ann_FR_search(ANNdist box_dist)
{
	...
	if (cut_diff < 0) {					// left of cutting plane
		...
		if (box_dist * ANNkdFRMaxErr < ANNkdFRSqRad || isNearlyEqual(box_dist * ANNkdFRMaxErr, ANNkdFRSqRad))
...
void ANNkd_leaf::ann_FR_search(ANNdist box_dist)
{
	...
	for (int i = 0; i < n_pts; i++) {	// check points in bucket
		...
		for(d = 0; d < ANNkdFRDim; d++) {
			...
			if( (dist = ANN_SUM(dist, ANN_POW(t))) > ANNkdFRSqRad &&  !isNearlyEqual(dist, ANNkdFRSqRad)) {
				break;
			}
		}
