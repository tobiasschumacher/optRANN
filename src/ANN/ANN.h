...
public:
  ...
  static inline double ANN_POW(double x)
  {
    if(ANN_NORM_P == 2.0)
      return x*x;
  ...
  static inline double ANN_ROOT(double x)
  {
    if(ANN_NORM_P == 2.0)
      return std::sqrt(x);
...
EPSILON_ABS			Absolute threshold up to which two doubles are
considered equal
EPSILON_REL			Relative threshold up to which two doubles are
considered equal
